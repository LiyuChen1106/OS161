...
load_elf(struct vnode *v, vaddr_t *entrypoint)
{
	...
	for (i = 0; i < eh.e_phnum; i++) {
		...
		if(count == 0){
			...
			as->as_segments[0].region = region;
			as->as_segments[0].p_offset = ph.p_offset;
			as->as_segments[0].p_filesz = ph.p_filesz;
			as->as_segments[0].p_flags = ph.p_flags & PF_X;
			count++;
		}
		else{

			as->as_segments[1].region = region;
			as->as_segments[1].p_offset = ph.p_offset;
			as->as_segments[1].p_filesz = ph.p_filesz;
			as->as_segments[1].p_flags = ph.p_flags & PF_X;
		}
...
//		//}
//
//		// store the segments info in as
//		assert(i == 0 || i == 1);
//		as->as_segments[i].p_offset = ph.p_offset;
//		as->as_segments[i].p_vaddr = ph.p_vaddr;
//		as->as_segments[i].p_memsz = ph.p_memsz;
//		as->as_segments[i].p_filesz = ph.p_filesz;
//		as->as_segments[i].p_flags = ph.p_flags & PF_X;
//
//	}
//
